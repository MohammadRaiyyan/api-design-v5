...
export async function createHabit(req: AuthenticatedRequest, res: Response) {
    try {
        ...
        const newHabit = await db.transaction(async (tx) => {
            const [habit] = await tx.insert(habits).values({
                userId,
                name,
                description,
                frequency,
...
export async function completeHabit(req: AuthenticatedRequest, res: Response) {
    try {
        ...
        await db.update(habits).set({
            ...
            and(eq(habits.id, habitId), eq(habits.userId, userId))
        );
    ...
    try {
        ...
        const habit = await db.transaction(async (tx) => {
            const [updatedHabit] = await tx.update(habits).set({
                ...
                and(
                    eq(habits.id, habitId), eq(habits.userId, userId)
                )
            ).returning();
            if (!updatedHabit) {
                res.status(404).json({ error: "Habit not found" });
            ...
            if (tagIds !== undefined) {
                await tx.delete(habitTags).where(
                    eq(habitTags.habitId, habitId)
                );
                if (tagIds.length > 0) {
